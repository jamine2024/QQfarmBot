...
function addItem(map, name, delta) {
    const key = name.trim();
    if (!key)
        return;
 ...
 */
function parseNameCountList(text) {
    return text
        .split("/")
        .map((s) => s.trim())
        .map((seg) => {
        ...
        .filter((x) => x !== null && Number.isFinite(x.count));
}
/**
 * 从日志中的 “收获/偷(作物明细)” 提取作物数量并累计。
 */
function applyFromHarvestDetail(counters, message) {
...
function addAction(counters, key, delta) {
    if (!Number.isFinite(delta) || delta <= 0)
        return;
    counters.actions[key] += delta;
}
function parseSummarySegments(text) {
...
function applyFromFarmLog(counters, message) {
    for (const seg of parseSummarySegments(message)) {
        if (seg.label === "浇水")
            addAction(counters, "water", seg.count);
        if (seg.label === "除虫" || seg.label === "捉虫")
            addAction(counters, "bug", seg.count);
        if (seg.label === "除草")
            addAction(counters, "weed", seg.count);
        if (seg.label === "收获")
            addAction(counters, "harvest", seg.count);
        if (seg.label === "种植")
            addAction(counters, "plant", seg.count);
        if (seg.label === "铲除")
            addAction(counters, "remove", seg.count);
    }
}
function applyFromFriendLog(counters, message) {
    for (const seg of parseSummarySegments(message)) {
        if (seg.label === "浇水")
            addAction(counters, "water", seg.count);
        if (seg.label === "除虫" || seg.label === "捉虫")
        ...
        if (seg.label === "除草")
            addAction(counters, "weed", seg.count);
        if (seg.label === "偷")
            addAction(counters, "steal", seg.count);
        if (seg.label === "放虫")
            addAction(counters, "putBug", seg.count);
        if (seg.label === "放草")
            addAction(counters, "putWeed", seg.count);
    }
}
function applyFromFertilizeLog(counters, message) {
    const m = message.match(/已为\s*(\d+)\s*\/\s*(\d+)\s*块地施肥/);
    if (!m)
        return;
    addAction(counters, "fertilize", Number(m[1]));
}
function applyFromPurchaseLog(counters, message) {
    const m = message.match(/已购买\s+(.+?)种子\s*x\s*(\d+)/);
    if (!m)
        return;
    addItem(counters.items, m[1], Number(m[2]));
}
function applyFromGoldLog(counters, message) {
...
function applyFromGainsLog(counters, message) {
    ...
    if (Number.isFinite(goldDelta) && goldDelta > 0)
        counters.gains.gold += goldDelta;
    if (Number.isFinite(expDelta) && expDelta > 0)
        counters.gains.exp += expDelta;
}
export class StatsStore {
    ...
    async load() {
        ...
        const loaded = await readJsonFile(this.filePath, base);
        this.counters = {
            ...base,
            ...loaded,
